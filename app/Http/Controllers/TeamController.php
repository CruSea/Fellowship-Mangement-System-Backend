...
class TeamController extends Controller
{
    ...
    public function assignMembers($name, $id) {
        try {
            ...
            if($contact->update()) { 
                return response()->json(['message' => 'team assigned to contact successfully'], 200);
            }
            return response()->json(['message' => 'unexpected error', 'error' => "Ooops! team doesn't assigned seccessfully"], 500);
        }catch(Exception $ex) {
            return response()->json(['message' => 'unexpected error', 'error' => 'Ooops! something went wrong'], 500);
        }
    }
